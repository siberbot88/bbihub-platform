<div class="space-y-6">
  ...
  <form wire:submit.prevent="save" class="rounded-xl border border-gray-200 bg-white p-6 shadow-sm space-y-5">
    ...
    <div>
      ...
      <input type="text" wire:model.defer="phone" placeholder="Masukkan nomor HP"
        class="w-full rounded-lg border-gray-300 focus:border-red-400 focus:ring focus:ring-red-100">
    ...
    @if ($role && $role !== 'superadmin')
      ...
      <div>
        ...
        {{-- OWNER: input biasa --}}
        @if ($role === 'owner')
          <input type="text" wire:model.defer="nama_bengkel" placeholder="Masukkan nama bengkel"
            class="w-full rounded-lg border-gray-300 focus:border-red-400 focus:ring focus:ring-red-100">
        @else
          {{-- ADMIN / MEKANIK: auto-complete workshop --}}
          <div class="relative">
            <input type="text" wire:model.debounce.500ms="nama_bengkel" placeholder="Ketik nama bengkel"
              class="w-full rounded-lg border-gray-300 focus:border-red-400 focus:ring focus:ring-red-100">

            {{-- dropdown suggestion --}}
            @if (!empty($bengkelOptions) && strlen($nama_bengkel ?? '') >= 2)
              <div
                class="absolute z-20 mt-1 w-full rounded-lg border border-gray-200 bg-white shadow-md max-h-64 overflow-auto">
                @forelse ($bengkelOptions as $bengkel)
                  <button type="button" wire:click="pilihBengkel('{{ $bengkel->id }}')"
      ...
      @if ($role === 'owner')
        ...
        <div>
          ...
          <textarea wire:model.defer="alamat" rows="3" placeholder="Masukkan alamat bengkel"
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div>
            ...
            <input type="text" wire:model.defer="province" placeholder="Masukkan provinsi"
              class="w-full rounded-lg border-gray-300 focus:border-red-400 focus:ring focus:ring-red-100">
          ...
          <div>
            ...
            <input type="text" wire:model.defer="country" placeholder="Masukkan negara"
              class="w-full rounded-lg border-gray-300 focus:border-red-400 focus:ring focus:ring-red-100">
      ...
      @elseif (in_array($role, ['admin', 'mechanic']) && $nama_bengkel)
        ...
        <div>
          ...
          <textarea rows="3" class="w-full rounded-lg border-gray-200 bg-gray-50 text-sm" readonly>{{ $alamat }}</textarea>
        </div>

        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">Kota</label>
            <input type="text" class="w-full rounded-lg border-gray-200 bg-gray-50 text-sm" readonly value="{{ $city }}">
          </div>
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">Provinsi</label>
            <input type="text" class="w-full rounded-lg border-gray-200 bg-gray-50 text-sm" readonly
              value="{{ $province }}">
          </div>
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">Negara</label>
            <input type="text" class="w-full rounded-lg border-gray-200 bg-gray-50 text-sm" readonly value="{{ $country }}">
          </div>
        </div>
      @endif
    @endif
